...
def get_site_config(host, testvars):
    ...
    site_config_string = host.file(site_config_path).content.decode('utf-8')
...
def test_takel_nginx_config_ssl_key(site_config, testvars):
    nginx_ssl_key = testvars['takel_nginx_ssl_key']
    config_ssl_key = re.search(r'ssl_certificate_key (.*);',
                               site_config['https'])
...
def test_takel_nginx_config_ssl_ciphers(site_config, testvars):
    nginx_ssl_ciphers = testvars['takel_nginx_ssl_ciphers']
    config_ssl_ciphers = re.search(r'ssl_ciphers (.*);', site_config['https'])
...
def test_takel_nginx_config_ssl_session_cache(site_config, testvars):
    nginx_ssl_session_cache = testvars['takel_nginx_ssl_session_cache']
    config_ssl_session_cache = re.search(r'ssl_session_cache (.*);',
                                         site_config['https'])
